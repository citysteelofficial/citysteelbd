  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!error) {
        ...
        alert(`Error deleting service: ${error.message || JSON.stringify(error)}`);
      }
    } catch (err: any) {
      console.error('Delete exception:', err);
      alert(`Exception deleting service: ${err.message}`);
    }
  };
                  ...
                  <tr key={item.id} className="border-b transition-colors hover:bg-muted/50">
                    ...
                    <td className="p-4 align-middle text-right">
                      <div className="flex items-center justify-end space-x-1">
                        <Link href={`/admin/services/${item.id}/edit`} className="inline-flex items-center justify-center rounded-md text-sm transition-colors hover:bg-muted h-9 w-9">
