...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "City Steel Corporation — The Best Industrial Steel Structure Solutions" },
      {
        name: "description",
        content:
          "City Steel Corporation delivers premium pre-engineered steel buildings, trusses, and industrial structures across Bangladesh.",
      },
      { property: "og:title", content: "City Steel Corporation — Industrial Steel Structures" },
      {
        property: "og:description",
        content: "Premium steel structure solutions for industrial construction.",
      },
    ],
  }),
  loader: async () => {
    const { data } = await supabase.from('blogs').select('*').eq('is_published', true).order('created_at', { ascending: false }).limit(5);
...
function HomePage() {
  const { latestBlogs } = Route.useLoaderData();
  const heroRef = useRef<HTMLElement | null>(null);

  // Subtle mouse-follow parallax on the hero (very small max offsets)
  useEffect(() => {
  ...
  return (
    <PageShell>
      ...
      >
        {/* Image: outer wrapper does slow Ken Burns zoom; inner img does mouse parallax */}
        ...
        {/* Soft red glow accent — bottom-left, gently floating + parallax */}
        <div
        ...
        <div className="container-x relative flex min-h-[68vh] flex-col justify-center py-20 lg:py-24">
          <div className="max-w-4xl">
            {/* Eyebrow */}
            <div className="mb-8 inline-flex items-center gap-3 border border-primary/50 bg-[oklch(0.13_0.01_20/0.55)] px-5 py-2.5 backdrop-blur-md">
              <span className="h-2 w-2 animate-pulse rounded-full bg-primary shadow-red" />
              <span className="text-[11px] font-bold uppercase tracking-[0.35em] text-[oklch(0.98_0_0)]">
                City Steel Corporation · Est. 2009
              </span>
            </div>

            {/* Headline */}
            <h1 className="font-display text-4xl font-black leading-[0.95] text-[oklch(0.99_0_0)] sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl">
              Building The
              <br />
            ...
            {/* Subtext */}
            <p className="mt-8 max-w-2xl text-lg leading-relaxed text-[oklch(0.85_0_0)] lg:text-xl">
            ...
            <div className="mt-12 flex flex-wrap items-center gap-5">
              <Link
                to="/contact"
                className="group inline-flex items-center gap-3 bg-gradient-primary px-9 py-5 text-sm font-bold uppercase tracking-[0.15em] text-primary-foreground shadow-red transition-all hover:scale-[1.03]"
              >
                Request a Quote
              ...
              <Link
                to="/projects"
                className="group inline-flex items-center gap-3 border border-[oklch(0.98_0_0/0.4)] bg-[oklch(0.13_0.01_20/0.4)] px-9 py-5 text-sm font-bold uppercase tracking-[0.15em] text-[oklch(0.98_0_0)] backdrop-blur-md transition-all hover:border-primary hover:bg-primary"
              >
                View Our Projects
                <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Link>
            ...
            {/* Trust strip */}
            <div className="mt-10 flex flex-wrap items-center gap-x-10 gap-y-4 border-t border-[oklch(0.98_0_0/0.15)] pt-6">
              {[
                { n: "500+", l: "Projects" },
                { n: "15+", l: "Years" },
                { n: "200+", l: "Clients" },
                ...
                <div key={s.l} className="flex items-baseline gap-2">
                  <span className="font-display text-2xl font-black text-[oklch(0.99_0_0)]">
                    {s.n}
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[oklch(0.7_0_0)]">
                    {s.l}
                  </span>
                </div>
              ))}
        ...
        <section className="border-t border-border bg-surface py-24">
          <div className="container-x">
            <div className="mb-12 flex items-end justify-between">
              ...
              <Link to="/blogs" className="hidden sm:inline-flex items-center gap-2 border border-border px-6 py-3 text-sm font-bold uppercase tracking-wider text-foreground transition-all hover:bg-primary hover:text-primary-foreground hover:border-primary">
                View All <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {latestBlogs.slice(0, 3).map((blog: any) => (
                <Link key={blog.id} to={`/blogs/$slug`} params={{ slug: blog.slug }} className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-soft transition-all hover:shadow-elegant">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    {blog.thumbnail_url ? (
                      <img src={blog.thumbnail_url} alt={blog.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    ) : (
            ...
            {latestBlogs.length > 3 && (
              <div className="mt-6 grid gap-6 md:grid-cols-2">
                {latestBlogs.slice(3, 5).map((blog: any) => (
                  <Link key={blog.id} to={`/blogs/$slug`} params={{ slug: blog.slug }} className="group flex overflow-hidden rounded-xl border border-border bg-card shadow-soft transition-all hover:shadow-elegant">
            ...
            <div className="mt-8 text-center sm:hidden">
              <Link to="/blogs" className="inline-flex items-center gap-2 bg-gradient-primary px-6 py-3 text-sm font-bold uppercase tracking-wider text-primary-foreground shadow-red">
      ...
      <section className="relative overflow-hidden border-t border-border bg-gradient-dark py-24 text-[oklch(0.98_0_0)]">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1 stripe-pattern" />
