...
export const Route = createFileRoute('/project/$slug')({
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: [
      ...
      { name: 'description', content: project.seo_description || project.description || '' },
    ...
    <PageShell>
      <section className="bg-surface py-20">
        <div className="container-x">
          <div className="mb-8">
            <Link to="/projects" search={{ status: project.status.toLowerCase() }} className="inline-flex items-center text-sm font-semibold text-muted-foreground hover:text-primary transition-colors">
          ...
          <div className="grid gap-12 lg:grid-cols-2">
              ...
              {allImages.length > 0 ? (
                <div className="space-y-3">
                  <div className="relative overflow-hidden rounded-2xl bg-muted border border-border shadow-elegant aspect-[4/3]">
                    <img src={allImages[activeIndex]} alt={project.title} className="w-full h-full object-cover transition-all duration-300" />
                    {allImages.length > 1 && (<>
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-auto bg-card rounded-xl border border-border p-8 shadow-soft">
                ...
                <Link to="/contact" search={{ subject: `Inquiry about project: ${project.title}` }} className="inline-flex items-center justify-center gap-2 rounded-md bg-gradient-primary px-8 py-4 text-base font-bold uppercase tracking-wider text-primary-foreground shadow-red transition-all hover:scale-[1.02] w-full sm:w-auto">
