...
export const Route = createFileRoute('/admin/services_/new')({ component: NewService });
...
function NewService() {
  ...
  const [formData, setFormData] = useState({ title: '', slug: '', category: 'complete-solution', description: '', content: '', seo_title: '', seo_description: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('services').insert([{ ...formData, image_url, images }]);
      ...
      <div className="rounded-xl border bg-card shadow">
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          <div className="space-y-3">
            <label className="text-sm font-medium">Service Images (Multiple)</label>
          ...
          <div className="flex justify-end pt-4"><button type="submit" disabled={loading} className="inline-flex items-center justify-center rounded-md bg-primary px-8 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{loading ? 'Uploading...' : 'Save Service'}</button></div>
        </form>
      </div>
    </div>
  );
