...
function EditProduct() {
  ...
  const [existingImages, setExistingImages] = useState<string[]>(product.images || (product.image_url ? [product.image_url] : []));
  const [formData, setFormData] = useState({
    title: product.title, slug: product.slug, category: product.category || 'steel-column',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('products').update({ ...formData, image_url, images: allImages }).eq('id', product.id);
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center space-x-2">
        <Link to="/admin/products" className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-input hover:bg-accent"><ArrowLeft className="h-4 w-4" /></Link>
        <h2 className="text-3xl font-bold tracking-tight">Edit Product</h2>
      </div>
      <div className="rounded-xl border bg-card shadow">
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2"><label className="text-sm font-medium">Product Name *</label><input required name="title" value={formData.title} onChange={handleChange} className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" /></div>
            ...
            <div className="space-y-2"><label className="text-sm font-medium">Category *</label>
              <select required name="category" value={formData.category} onChange={handleChange} className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                <option value="steel-column">STEEL COLOUM</option><option value="steel-beam">STEELL BEAM</option><option value="purlin">PURLIN</option>
                <option value="sliding-door">SLIDING DOOR</option><option value="roof-monitor">ROOF MONITOR</option><option value="ms-plate">MS PlATE</option>
                <option value="industrial-deck-sheet">INDUSTRIAL DECK SHEET</option><option value="industrial-roof-sheet">INDUSTRIAL ROOF SHEET</option>
                <option value="self-drilling-screw">Self-Drilling Screw</option><option value="rafter">RAFTER</option>
              </select>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-4"><button type="submit" disabled={loading} className="inline-flex items-center justify-center rounded-md bg-primary px-8 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{loading ? 'Saving...' : 'Update Product'}</button></div>
        </form>
      </div>
    </div>
  );
