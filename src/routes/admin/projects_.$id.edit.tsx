...
function EditProject() {
  ...
  const [existingImages, setExistingImages] = useState<string[]>(project.images || (project.image_url ? [project.image_url] : []));
  const [formData, setFormData] = useState({
    title: project.title, slug: project.slug, status: project.status,
    location: project.location || '', description: project.description || '',
    content: project.content || '', seo_title: project.seo_title || '', seo_description: project.seo_description || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('projects').update({ ...formData, image_url, images: allImages }).eq('id', project.id);
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <div className="rounded-xl border bg-card shadow">
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2"><label className="text-sm font-medium">Status *</label>
              <select required name="status" value={formData.status} onChange={handleChange} className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                <option value="COMPLETED">Completed Project</option><option value="ONGOING">Ongoing Project</option>
              </select>
            </div>
          ...
          <div className="flex justify-end pt-4"><button type="submit" disabled={loading} className="inline-flex items-center justify-center rounded-md bg-primary px-8 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{loading ? 'Saving...' : 'Update Project'}</button></div>
        </form>
      </div>
    </div>
  );
