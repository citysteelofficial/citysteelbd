...
export function ProductGallery({ products }: { products: Product[] }) {
  ...
  // Flatten all images from all products into a single array
  const galleryItems: GalleryItem[] = [];
  
  products.forEach((product) => {
    // Get all unique images for this product
    const productImages = new Set<string>();
    if (product.image_url) productImages.add(product.image_url);
    if (product.images && Array.isArray(product.images)) {
      product.images.forEach(img => productImages.add(img));
    }
    
    // Create a gallery item for each image
    Array.from(productImages).forEach((url, index) => {
      if (url) {
        galleryItems.push({
          id: `${product.id}-${index}`,
          url,
              ...
              <div className="mt-6 flex-1 space-y-4">
                {selectedItem.product.content ? (
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: selectedItem.product.content }}
                  />
                ) : (
